...
extension APIRequest {
    func toUrlRequest(data: APIRequest) throws -> URLRequest  {
        ...
        if let _ = urlComponents?.queryItems {
            urlComponents?.queryItems?.append(URLQueryItem(name: "appid", value: data.netWorkConfiguration.apiKey))
        } else {
            urlComponents?.queryItems = [URLQueryItem(name: "appid", value: data.netWorkConfiguration.apiKey)]
        }
        url = (urlComponents?.url)!
